...
export function AgentNode({ id, data, selected }: NodeProps) {
  ...
  return (
    ...
    >
      ...
      {logs && logs.length > 0 && (
        <div className={`mt-2 bg-slate-50 border border-slate-100 rounded-lg p-2 font-mono text-[10px] text-slate-600 space-y-1 transition-all ${status === 'running' ? 'ring-1 ring-teal-200 ring-offset-1' : ''}`}>
          <div className="flex justify-between items-center mb-1 pb-1 border-b border-slate-100 text-[9px] text-slate-400 uppercase tracking-tighter">
            ...
            {status === 'running' && <span className="text-teal-500 animate-pulse">Streaming...</span>}
          </div>
          {logs.slice(-3).map((log: any, i: number) => (
            <div key={`${log.type}-${i}`} className="flex animate-fade-in-up">
              <span className="text-teal-600 mr-1 font-bold shrink-0">[{log.type}]</span>
      ...
      {status === 'running' && (
        <div className="w-full bg-slate-100 rounded-full h-1 mt-3 overflow-hidden relative">
          <div className="node-loading-bar absolute inset-y-0 left-0 w-1/3 bg-teal-500 rounded-full" />
        </div>
      )}
    </BaseNode>
  );
